...
let startGameServer port guiPort =
  ...
  let rec startNewGame () =
    ...
    let rec waitAndPop () =
      ...
      else performTSRefOp (fun l -> match !l with h::t -> (l := t; h) | _ -> failwith "unknown error")  players playersMutex
    in
    ...
    let rec handleConnection c client =
      ...
      while !cont do
      (try (
        cont := match Connection.input client with
                  Some(Action(id, a)) ->
                      let res = Game.handleAction game id a c in
      ...
      with e -> print_endline (Printexc.to_string e); performTSRefOp (fun lc -> lc := Some c) lostConn lostConnMutex; cont := false)
      done
    in
    ...
    let rec handleTime () =
      ...
      | Some c -> Netgraphics.add_update(GameOver(c)); Netgraphics.send_updates(); performTSRefOp (fun grr -> grr := Some c) gameResult gameResultMutex
    in
    ...
    let rec waitForGameOver() =
      ...
      let isGr = isGr ||
        match lostConn with
          Some(c) -> performTSRefOp (fun gro -> gro := Some (Winner (c))) gameResult gameResultMutex; true
        | _ -> isGr
        in
      let _ = if isGr then
